...
class myns3env(gym.Env):
 ...
 #   self.state_short = 24
  ...
  def step(self, action_TD3):
    ...
    action_DDQN=np.base_repr(action_index+int(self.a_level)**int(self.a_num),base=int(self.a_level))[-self.a_num:]# decoding the action index to the action vector
    action_DDQN=[int(action_DDQN[s]) for s in range(len(action_DDQN))]
    #action_DDQN=np.concatenate((np.zeros(a_num-len(action_DDQN)),action_DDQN),axis=None)
    action_DDQN=[step_MIMO*x for x in action_DDQN]#action vector
    #action_DDQN=[0,0,0]#SISO only
    #action_DDQN=[1,1,1]#MIMO only
    print("mimo_action" , action_DDQN)
    DDQN_action_total = sum(action_DDQN)  / len(action_DDQN)                                   # <<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>
    print ("MIMO_ total ",sum(action_DDQN) )
    ...
    #Whole action
    big_action = np.concatenate((action_TD3,action_DDQN),axis=None)
...
##############
    ...
    state5=np.reshape(action_DDQN,[self.Cell_num, 1])
    ...
    if len(reward1) % self.max_env_steps == 0:
        ...
        with open('Rewards_' + self.time_var + '.csv', 'w', newline='') as rewardcsv:
            ...
            results_writer.writerow(Result_row)
            Result_row.clear()
            Result_row=Result_row+avg_MIMO.tolist()       #row23  averge MIMO ACtion total
            results_writer.writerow(Result_row)
        rewardcsv.close()
