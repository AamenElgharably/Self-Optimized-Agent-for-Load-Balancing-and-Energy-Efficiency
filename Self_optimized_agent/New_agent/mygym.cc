...
MyGymEnv::calculate_rewards()
{
         std::map<uint16_t , std::map<uint16_t, float>>::iterator itr;
          ...
          for (itr = UserCQI.begin(); itr != UserCQI.end(); itr++)//cells loop
          {
      	  ...
      	  m_UesNum.assign(m_cellCount, 0);
		 float avguserthr=0;
	     std::cout <<  " DZ_Users_num "<<DZ_Users_num<<std::endl;
          ...
          for (itr = UserThrouput.begin(); itr != UserThrouput.end(); itr++)
          {
			  ...
			  std::cout<<"ratio of users not satisfying threshold in cell   "<<itr->first<< "    is   "<< counter[itr->first-1]<< std::endl;
          }
		  
          Blocked_Users_num+=DZ_Users_num;
          std::cout <<"Number of users "<<m_UesNum.at(0)+m_UesNum.at(1)+m_UesNum.at(2)+m_UesNum.at(3)+m_UesNum.at(4)+m_UesNum.at(5)<<std::endl;
          std::cout  << "Number of blocked users: "<<Blocked_Users_num << std::endl;
          NS_LOG_UNCOND ("Number of blocked users: "<<Blocked_Users_num);
		  
          NS_LOG_UNCOND ("Average users throughput: "<<avguserthr);
          std::cout << "Average users throughput:  " << avguserthr<<std::endl;
...
MyGymEnv::ExecuteActions(Ptr<OpenGymDataContainer> action)
{
  ...
  std::cout<<"Execute Actions"<< actionVector.size() << std::endl;
 ...
 for(size_t index=0;index!=m_cells.GetN();++index)
 {
	 if (mimo_actionVectord[index] == 1)
				mimo_ratio[index] +=1;
		std::cout<<"mimo ratio  "<< mimo_ratio[index]<<std::endl;	
 }
  ...
  CellIndividualOffset::setOffsetList(cio_actionVectord);     //execute CIO action
	...
	
	{
		...
		std::cout<<"End of Execute Actions"<<std::endl;
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
